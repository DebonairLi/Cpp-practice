...
with open(dataset, "r") as f:
    ...
    for data in reader:
        ...
        click_time = 0
        if opt.dataset == 'yoochoose':
            click_time = time.mktime(time.strptime(curdate[:19], '%Y-%m-%dT%H:%M:%S'))
            click_time += float(curdate[20:23]) / 1000
        else:
            click_time = time.mktime(time.strptime(curdate, '%Y-%m-%d'))
            click_time += float(data['timeframe']) / 1000 

        curid = sessid
        if opt.dataset == 'yoochoose':
            item = data['itemId'], click_time
        else:
            item = data['itemId'], click_time
